...
import { Mail, Lock, Eye, EyeOff, ArrowRight, GraduationCap, User, Building2, BookOpen, QrCode, DoorOpen, Calculator, Sun, Moon } from 'lucide-react';
...
export default function AuthPage({ initialView = 'login' }) {
  ...
  const { register: regLogin, handleSubmit: handleLoginSubmit, formState: { errors: loginErrs, isSubmitting: isLoginSubmitting } } = useForm({ resolver: yupResolver(loginSchema) });

  // Register State
  const [regErr, setRegErr] = useState('');
  const [regSuccess, setRegSuccess] = useState(false);
  const { register: regSignup, handleSubmit: handleSignupSubmit, formState: { errors: regErrs, isSubmitting: isRegSubmitting } } = useForm({ resolver: yupResolver(registerSchema), defaultValues: { role: 'student' } });

  useEffect(() => { initTheme(); }, []);

  useEffect(() => {
    // Sync flip state with URL changes
    ...
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <img 
          ...
          className="w-full h-full object-cover mc-fade-in"
        />
      ...
      <div className="relative z-10 w-full max-w-[1200px] flex flex-col lg:flex-row items-center justify-center gap-12 p-4 sm:p-6 lg:p-12">

        {/* ════════════════════════════════════════════
            LEFT PANEL — The Info Rectangle (Hidden on mobile)
        ════════════════════════════════════════════ */}
        <div className={`hidden lg:flex flex-col w-[380px] h-[600px] rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl ${dark ? 'dk-card border border-[#c9a84c]/20' : 'glass-card border-[1.5px] border-indigo-200/60'}`}>
          ...
          <div className="flex-1 flex flex-col justify-center">
            <h2 className={`mc-fade-up text-4xl font-extrabold leading-[1.15] mb-4 tracking-tight ${dark ? 'text-white' : 'text-gray-900'}`}>
              {isFlipped ? (
                <>Join your<br/><span className={dark ? 'mc-gold-shimmer' : 'mc-gradient-text'}>campus.</span></>
              ) : (
                <>Welcome<br/><span className={dark ? 'mc-gold-shimmer' : 'mc-gradient-text'}>back.</span></>
              )}
            </h2>
        ...
        {/* ════════════════════════════════════════════
            RIGHT PANEL — The 3D Rotating Circle
        ════════════════════════════════════════════ */}
        ...
        <div className="relative w-[340px] h-[340px] sm:w-[480px] sm:h-[480px] lg:w-[640px] lg:h-[640px] flex items-center justify-center" style={{ perspective: '2000px' }}>
          ...
          >
            ...
            {/* ================= FRONT: LOGIN CIRCLE ================= */}
            <div 
              className="absolute inset-0 rounded-full flex flex-col items-center justify-center p-16"
              style={{
            ...
            >
              ...
              <div className="w-[340px] text-center relative z-10">
                ...
                <form onSubmit={handleLoginSubmit(onLogin)} className="space-y-5 text-left">
                  <div>
                    <div className="relative">
                      <Mail size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                      <input {...regLogin('email')} type="email" placeholder="you@vitbhopal.ac.in" className={`${inputBase} pl-12`} style={lightInputStyle} />
                    </div>
                    {loginErrs.email && <p className="mt-1.5 pl-4 text-xs text-red-500 font-medium">{loginErrs.email.message}</p>}
                  </div>

                  <div>
                    <div className="relative">
                      <Lock size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                      <input {...regLogin('password')} type={showPass ? 'text' : 'password'} placeholder="••••••••" className={`${inputBase} pl-12 pr-12`} style={lightInputStyle} />
                      <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                        {showPass ? <EyeOff size={16} /> : <Eye size={16} />}
                      </button>
                    </div>
                    {loginErrs.password && <p className="mt-1.5 pl-4 text-xs text-red-500 font-medium">{loginErrs.password.message}</p>}
                  </div>

                  <button type="submit" disabled={isLoginSubmitting}
                    className={`mc-btn w-full py-4 rounded-full text-base font-bold disabled:opacity-60 mt-6 shadow-xl`}
                    style={dark ? {
                      background: 'linear-gradient(135deg, #c9a84c 0%, #a87c30 100%)', color: '#07070f',
            ...
            >
              ...
              <div className="w-[380px] text-center relative z-10">
                ...
                <form onSubmit={handleSignupSubmit(onRegister)} className="space-y-3.5 text-left">
                  
                  {/* Name & Role in one row to save vertical space */}
                  <div className="flex gap-3">
                    <div className="relative flex-1">
                      <User size={15} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                      <input {...regSignup('name')} type="text" placeholder="Full name" className={`${inputBase} pl-10 py-3`} style={lightInputStyle} />
                    </div>
                    
                    <div className="flex bg-[#07070f]/20 rounded-full p-1" style={lightInputStyle}>
                      {['student', 'faculty'].map((r) => (
                        <label key={r} className="relative cursor-pointer">
                          <input {...regSignup('role')} type="radio" value={r} className="sr-only peer" />
                          <div className={`px-3 py-2 rounded-full text-xs font-semibold capitalize transition-all ${
                            dark
                              ? 'text-gray-500 peer-checked:bg-[#c9a84c]/20 peer-checked:text-[#c9a84c]'
                  ...
                  <div>
                    <div className="relative">
                      ...
                      <input {...regSignup('email')} type="email" placeholder="you@vitbhopal.ac.in" className={`${inputBase} pl-12 py-3`} style={lightInputStyle} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="relative">
                      ...
                      <input {...regSignup('password')} type={showRegPass ? 'text' : 'password'} placeholder="Password (min 6 chars)" className={`${inputBase} pl-12 py-3 pr-12`} style={lightInputStyle} />
                      <button type="button" onClick={() => setShowRegPass(!showRegPass)} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400">
                  ...
                  <div>
                    <div className="relative">
                      ...
                      <select {...regSignup('department')} className={`${inputBase} pl-12 py-3 appearance-none`} style={lightInputStyle}>
                  ...
                  <button type="submit" disabled={isRegSubmitting || regSuccess}
                    className={`mc-btn w-full py-3.5 rounded-full text-base font-bold disabled:opacity-60 mt-2 shadow-xl`}
                    style={dark ? {
                      background: 'linear-gradient(135deg, #c9a84c 0%, #a87c30 100%)', color: '#07070f',
                      boxShadow: '0 10px 25px rgba(201,168,76,0.3)'
                    } : {
