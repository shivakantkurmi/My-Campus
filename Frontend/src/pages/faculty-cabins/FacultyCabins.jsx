...
import { Search, Plus, Pencil, Trash2, DoorOpen, MessageSquarePlus } from 'lucide-react';
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search faculty or department…"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(c => (
          <div key={c._id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 hover:shadow-md transition">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-semibold text-gray-800 dark:text-white">{c.facultyName}</h3>
                <p className="text-xs text-blue-500 mt-0.5">{c.department}</p>
              </div>
              {isAdmin && (
      ...
      {feedback === 'open' && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            {fbSent ? (
              <p className="text-green-500">Feedback sent to admin!</p>
            ) : (
              <>
