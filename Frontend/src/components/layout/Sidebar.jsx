    ...
    <aside className="w-[100px] h-full py-6 pl-6 lg:flex flex-col hidden z-30 transition-all">
      <div className="bg-white/60 backdrop-blur-[40px] border-[2px] border-white/90 rounded-[2rem] w-[80px] h-full flex flex-col items-center py-8 shadow-[0_30px_80px_-15px_rgba(255,255,255,0.6)] relative">
        ...
        <div className="mt-auto flex flex-col items-center gap-3">
          <Link to="/privacy-policy" className="w-10 h-10 rounded-full flex items-center justify-center text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Privacy Policy">
            <ShieldCheck size={18} />
          </Link>
...
function DarkSidebar({ user, onClose, handleLogout, filtered }) {
  return (
    <aside className="w-64 h-full bg-[#08080f]/95 backdrop-blur-xl border-r border-[#c9a84c]/10 flex flex-col lg:flex hidden z-30 transition-all shadow-[4px_0_32px_rgba(0,0,0,0.6)]">
      ...
      <div className="px-4 py-6 flex flex-col gap-2 mt-auto">
        <Link to="/privacy-policy" className="flex items-center gap-4 px-4 py-2 text-xs font-semibold text-gray-500 hover:text-gray-300 hover:bg-white/5 rounded-2xl transition-colors">
          <div className="w-8 h-8 rounded-full flex items-center justify-center bg-white/5"><ShieldCheck size={16} /></div>
          Privacy Policy
        </Link>
        <button onClick={handleLogout} className="flex items-center gap-4 px-4 py-2.5 text-sm font-bold text-gray-500 hover:text-rose-400 hover:bg-white/5 rounded-2xl transition-colors">
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 z-50 flex flex-col transition-transform duration-300 lg:hidden ${
        ...
        <div className={`flex items-center justify-between px-5 py-4 border-b ${dark ? 'border-[#c9a84c]/10' : 'border-gray-100'}`}>
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              dark ? 'bg-gradient-to-br from-[#c9a84c] to-[#8a6020]' : 'bg-indigo-600'
            }`}>
              <GraduationCap size={16} className="text-white" />
            </div>
            <span className={`font-bold ${dark ? 'text-[#c9a84c]' : 'text-gray-900'}`}>My-Campus</span>
          </div>
        ...
        <div className={`px-4 py-4 border-t ${dark ? 'border-[#c9a84c]/10' : 'border-gray-100'} flex flex-col gap-1`}>
           <Link to="/privacy-policy" onClick={onClose} className={`flex items-center gap-3 w-full px-3 py-2 rounded-xl text-xs font-semibold ${dark ? 'text-gray-400 hover:bg-white/5' : 'text-gray-600 hover:bg-gray-50'}`}>
             <ShieldCheck size={16} /> Privacy Policy
           </Link>
           <button onClick={handleLogout} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-bold ${dark ? 'text-rose-400 hover:bg-white/5' : 'text-red-500 hover:bg-red-50'}`}>
